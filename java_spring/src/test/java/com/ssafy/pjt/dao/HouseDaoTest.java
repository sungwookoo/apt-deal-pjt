...
public class HouseDaoTest extends AbstractTest {
	private static final Logger logger = LoggerFactory.getLogger(HouseDaoTest.class);
	...
	public void selectGugunTest() throws SQLException {
		...
		List<Map<String, String>> result = houseDao.selectGugunNames(sidoCode);
		logger.debug("--------------start3");
	...
	public void selectDealsTest() throws SQLException {
		...
		List<HouseDto> result = houseDao.selectDeals(map);
		assertEquals(310, result.size());
		
		map.put("year", "2015");
		map.put("month", "1");
		result = houseDao.selectDeals(map);
